...
init: install ## Make full application initialization (install, migrate, seed, build assets, etc)
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" php bin/console doctrine:migrations:migrate --allow-no-migration --no-interaction -vvv
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" php bin/console doctrine:fixtures:load --no-interaction -vvv

test: up ## Execute application tests
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" bin/phpunit